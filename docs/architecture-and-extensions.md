...
> **写工具的具体契约**（manifest 逐字段、postMessage 协议、op 全表与上限、
> 注入组件的写法、报错对照表）在另一份：[**《工具编写与注入 · 详细说明》**](tool-authoring-and-injection.md)。
> 这份文档讲**为什么这么设计**，那一份讲**照着怎么做**。
> 可抄的样本：[`examples/kitchen-sink/`](../examples/kitchen-sink/)（每条通道都真跑一遍）。
