...
export interface ToolManifest {
  ...
  schema?: ToolSchema;
  /**
   * 要向宿主申请的**共享能力**（目前只有 "gallery"）。
   *
   * 为什么单独声明而不是"装了就能用"：能力碰的是**不属于这个工具的数据**
   * （图库是全局素材库），应当像权限一样显式写在门口 —— 设置页要能列出
   * "这个工具会碰图库"，宿主也能据此拒绝没声明过的调用。
   * 私有表不需要声明在这里（那是 `schema`，工具自己的东西）。
   */
  capabilities?: string[];
  /**
   * 这个工具要**嵌进宿主界面**的位置（注入组件）。
   *
   * 不写就只是"工具区里的一个整页工具"；写了之后它还会出现在待办详情
   * 面板 / 详情头部 / 列表行上，宿主把当前那条待办的上下文投给它。
   * 形状见 lib/extensions/types.ts 的 ToolInjectSpec —— **外部输入**，
   * 只认白名单里的三个 kind，认不出来的一律丢掉。
   */
  injects?: ToolInjectSpec[];
  /**
   * 这个工具**愿意接受哪些命令**（助手驱动它干活用的，见 lib/agent/toolRuntime.ts）。
   *
   * 为什么必须声明：不声明的话助手可以对任何工具发任何字符串，
   * 而"它听不听得懂"变成一件只有运行时才知道的事 —— 宿主也说不清
   * 这次调用到底成功了没有。声明了，宿主就能在发出去之前把不认识的
   * 命令挡回来，并把"它有哪些命令"作为一条可读的信息给出去。
   *
   * 形状：命令名的字符串数组，如 ["start", "reset"]。**外部输入**，
