...
> 想往里加东西？先读这两份：
>
> - [**《架构与扩展开发指南》**](docs/architecture-and-extensions.md) —— 内核边界、模块注册表、
>   注入点契约、能力申请与降级、数据通道与权限，以及「加一个模块 / 加一个工具各要动哪些地方」的清单。
> - [**《工具编写与注入 · 详细说明》**](docs/tool-authoring-and-injection.md) —— **写工具的人看这份**：
>   manifest 逐字段、postMessage 协议、18 个 op 的参数与上限、三个注入位置、能力门与降级、
>   沙箱与通行证、报错对照表。
>
> 配一份把每条通道都真跑一遍的样本：[**`examples/kitchen-sink/`**](examples/kitchen-sink/)。
...
- **特殊单号**（**选装模块**：设置 → 行为 → 模块里打开，默认不带）以快递单号起算，
  时效 =「距下一步骤的剩余时间」，流程可定制。打开后专属视图、创建时的类型选择、
  「我的一天」里那一组、时效提醒与紧急区一起出现；关掉只是收起入口与提醒，
  已经建好的单子仍留在「流程任务」里
- 逾期与临期各提醒一次，临近过期红色高亮
...
**主题色调是两个色，不是一个。** `accent`（品牌色）用在待办完成圈、紧急标记、AI 悬浮球这类
「属于本应用」的印记上；`primary`（主操作色）用在按钮、选中态、输入框聚焦边框这类
「能点的东西」上。它们同屏出现，合成一个色就分不清哪是标记、哪能点了。
换色换的是**整套关系**（8 套预设，或自己调），不是单个色值。

落地方式是启动时把色值写进 `<html>` 的内联 style（`--color-accent` / `--color-primary`），
优先级高于 `@theme` 里的初始值 —— 所以改色不需要重新构建，也不需要 Tailwind 预生成类名。

⚠️ **可选范围被刻意限制**：界面上 `bg-primary` / `bg-accent` 上的字是白色，
所以任何进入 CSS 的颜色都会先过 `fitForUi()`，收到「白字站得住」的相对亮度以内
（纯黄这类色在 HSL 里明度只有 0.5，感知亮度却有 0.93，光按 HSL 钳制会放过它）。
这比为了支持浅色主题色去改 30 多处 `text-white` 省心，也不会漏。

壁纸有两种来源：

| 来源 | 存在哪 | 说明 |
|---|---|---|
| 必应每日壁纸 | `public/wallpapers/`（随包静态资源） | 离线优先，不联网取图；换一批跑一次 `scripts/fetch-wallpapers.mjs` |
| **自己上传的图** | 附件仓库（桌面 AppData / 浏览器 IndexedDB） | 单张 ≤ 12 MB、只收图片；设置里只存仓库相对路径 |

自己传的图走 `attachmentStore`，跟图库、附件同一套仓库 —— 同样的「我传进来的文件」
应该在同一处，清理和查占用都复用现成的入口，而不是让用户在两个地方找。

**从壁纸取色**是手动的：铺着图时点「取这张图的主色」，把图缩到 96px 分桶取样，
按「占比 × 饱和度」挑出前两簇，大的那簇当品牌色、第二簇当主操作色
（两簇拉不开时按色相转 150° 造一个）。没做 k-means —— 这里要的是「这张图的整体印象色」，
而 k 取多少都是拍脑袋，且随机初始中心会让同一张图每次算出不同结果。
...
仓库里有一份《单 HTML 工具编写标准》的**活样本**：五子棋
（`tests/fixtures/gomoku/`），单文件自包含 HTML、只走 postMessage、
战绩写自己的私有表、深浅色跟随宿主。它是当时为了验证"AI 助手能不能按标准
自己写出工具"而生成的**开发样本**，不随安装包与工具包分发 —— 所以它不住在
`tools/`（那里的每个目录都会出现在用户的设置页里、会进工具包 zip），
而是放在测试夹具目录；它的价值在 `npm run gomoku:test` —— 那 59 项断言把
标准的硬规则**逐条钉死**（自包含 / 无外部引用 / data-* 齐备 / 响应宿主主题 /
...
**没接过模型时它会告诉你从哪儿开始。** 助手需要一个对话接口才能说话，而调用接口需要一把
API Key —— 卡住新手的从来不是"填哪一格"，而是**Key 从哪儿来**。所以窗口和设置页
（设置 → AI 助手）都会给出同一条路：先推荐一家自带注册地址的服务商，注册 → 建 Key →
粘回来；手上已经有别家 Key 的人照旧自选，四家厂商与自建网关都接得上。

**不想用可以整个关掉**：设置 → AI 助手里的「启用 AI 助手」是总闸。关掉后悬浮球不再出现、
不加载也不会联网；聊过的记录留着，再打开就原样回来（开关沿用统一的 `ext.agent.enabled`）。
...
| 能力 | 它到底做了什么 | 权限开关 |
|---|---|---|
| 按标准写工具 / 写组件 | 照内嵌的标准写一个单文件工具（或嵌进待办详情的组件），**先过沙箱**再装进工具目录并打开 | 写工具 |
| 建日程 / 改 / 删 | 往待办里写条目与子任务、改标题日期提醒清单、删条目（含子任务） | 建日程 |
| 改子任务 | 给某条待办追加 / 改 / 删 / 勾选子任务 | 建日程 |
| 绑数据表 | 给**自己导入的**工具写私有表声明（不碰内置工具：升级会覆盖） | 绑数据表 |
| 卸载 / 恢复工具 | 卸掉一个工具（数据留着），或把内置工具恢复成出厂版本 —— **要先经你确认** | 写工具 |
| 读写工具的私有表 | 往某个工具**自己声明过的**表里导数据、查记录、按 id 删一条 | 绑数据表 |
| 让工具去干一件事 | `call_tool`：给已打开的工具发一条它**声明过**的命令（开始计时、清零…） | 不需要 |
| 写文件 | 报告 / 方案 / markdown 写进它的**工作区**目录，长文不占对话 | 写工具 |
| 自己记一条技能 | `add_skill`：把它总结的规矩存进库，以后每轮都带上（下次不会忘） | 不需要 |
...
（五份内置技能：写工具 / 绑数据表 / 写注入组件 / 算日期 / 接工具，加上它自己记的）。
标着「它自己记下的」那几条可以删 —— 内置的删不掉，那是"怎么写才装得上"的标准。
注入给模型的是**每条一句话的硬规则**，全文按需用 `read_skill` 取，避免一上来就把上下文塞满。

#### 工作区：它写文件的地方

助手有一个自己的工作区目录（默认是数据目录下的 `agent-workspace/`，可以在
设置 → AI 助手 里改成任何目录，比如同步盘）。报告、方案、整理好的清单、markdown
都写进去，**而不是在对话里整篇回贴**。

打开「设置 → 行为 → 模块」里的**工作区**就能看到它们：左边列文件，右边直接渲染
markdown，旁边有「在资源管理器中打开」。

边界是物理的：路径只接受**相对工作区根目录**的相对路径，`..`、绝对路径、盘符一律拒 ——
那是"助手把文件写到别处"的唯一通道。默认也不覆盖已有文件（要覆盖得显式说）。
浏览器演示模式没有可写的工作区，那时它直接说明做不到。

##### 长文件：一次写不完就分段写

一次回复**有输出长度上限**。写一份几百行以上的工具 HTML，写到一半会被掐断 ——
那时代码块没有闭合，宿主其实什么都没收到，而模型**看不见 `finish_reason`**，
它以为自己写完了，下一步就拿着半截源码去装，宿主报「文件是空的」，它读成
"我参数写错了"，改完参数名原样再来一遍，又断在同一个地方（2026-09-26 真机：
卡了 6 轮才装上，根因就是这个循环）。

所以这条路现在是三段式的：

1. **请求里显式带上输出上限**（不吃服务端默认 4096）；
2. **判得出**：`finish_reason === "length"` 时记为"被掐断"，代码块没闭合也判得出来；
3. **回给它**：被掐断时往对话里回一句"你上一条没写完，接着写，别从头再来"，
   并且给出走得通的那条路 —— `write_file` **分段写**（第一段正常写，之后每段带
   `append: true`），整份写完之后 `sandbox_run` / `install_tool` 只给 `html_file`
   指向那个文件，源码不必再贴一遍。

提示词里也提前写了这件事（"一次回复有长度上限 → 分段 write_file"），
因为**事先知道有这条路**比事后被纠正少走两三轮。

#### 工具箱：写 → 沙箱试跑 → 拿通行证 → 提交

助手写的代码是**机器批量产出的，没有人在它落盘之前看过一眼**，所以这条路自带
一道机器关，四步一步都不能跳：

| 步骤 | 动作 | 发生了什么 |
|---|---|---|
| 1. 写 | — | 按《单 HTML 工具编写标准》或《注入组件》标准写一份自包含的 HTML |
| 2. 试跑 | `sandbox_run` | 放进**隔离 iframe 真跑一遍**：抓控制台报错与未捕获异常、看界面是不是白屏、核对其声明的能力与它实际调用的桥接对不对得上 |
| 3. 拿票 | — | 通过了才发一张 `ticket`；没通过就把每一条问题（含怎么改）列出来 |
| 4. 提交 | `install_tool` | 带上票才准装。**没票、票过期、源码改过后票对不上，一律直接拒绝** |

试跑**不落库**（`row.*` / `kv.*` 只记账，图库不真存），但能力门与正式运行完全一致：
没申请 `gallery` 却调 `gallery.put`，在沙箱里一样被拒 —— 于是这类问题在装之前就暴露了。
通行证绑的是 (id, 源码, schema, 能力, 注入位置) 的指纹，**源码改一个字节票就作废**，
所以它不可能"验一份、装另一份"。指纹里 schema 的**键顺序不影响结果**（模型在 install
那一步把 schema 重写一遍是常事，内容一字不差却因键序不同而作废太冤）。

源码与票对不上时，它不会直接拒绝到底，而是**当场把新的这份重新验一遍**：过了就装新的
（它此刻的意图），不过才拒绝并把问题列出来。前提是这个环境**真的跑得起来沙箱** ——
跑不起来时不放行，因为半截的 HTML 照样能通过静态体检，那等于把坏文件装上去。
通行证这道门没有被绕过去：没票、票不存在、票过期、id 对不上，一律照旧拒绝。

这道门落在宿主里，不在提示词里 —— 与"删除前先问你"是同一个道理：
提示词只是概率，而没验过的代码落到你磁盘上这件事不该靠概率。
有些环境跑不了 iframe（比如 Node 里跑单测），那时它只过静态体检，票上**如实记着
"没试跑过"**，标准本身（有错就不给票）不因环境而变。
...
| 图库 | 图库**记录**（图库本身是选装模块，没开就不会有东西可同步） |
| 附件 | 流程任务附件的**记录** |
...
| 14 | `add_sync_timestamps` | 同步用的时间戳 |
| 15 | `add_agent_messages` | AI 助手消息 |
| 16 | `add_agent_chats` | 助手多会话（消息挂 `chat_id`，老会话回填到 `chat-legacy`） |
| 17 | `modules_opt_in` | 特殊单号与图库改为选装（开关统一到 `ext.<id>.enabled`）：用过哪个就给哪个把开关打开 |
| 18 | `agent_skills` | 助手自己写的技能落库（`core_agent_skills`）：内置是代码，它自己总结的规矩要能留下来 |
...
main/
  src/
    lib/
      ...
      attachments.ts   附件
      notify.ts        系统通知
      wallpapers.ts    壁纸（内置清单 + 背景设置的解析；纯函数，可在 Node 里直接 import）
      customWallpaper.ts 自定义壁纸的存取（走附件仓库；与 wallpapers.ts 分开是为了不把它拖进 Node 环境）
      theme.ts         主题色：色值算术、预设色板、**可用区间**（保证白字可读）、写到 CSS 变量
      palette.ts       从图片取主题色（缩略分桶取两簇；纯函数与取像素分开）
      useWallpaperUrl.ts 背景设置 → <img src>（自定义那半是异步的）
      icons.ts         图标名映射
      ...
      syncClient.ts    同步编排（读配置、跑分片、出报告）
        agent/           内置 AI 助手
          ...
          actions.ts       十五个动作的执行、**唯一**权限门、**强制确认门**与**通行证门**
          protocol.ts      动作协议（原生 tool_calls + workbench 代码块）
          sandbox.ts       沙箱：隔离 iframe 真跑候选源码 + 影子桥（不落库）+ 探针报告
          verifier.ts      验证器：静态体检 / 报告判定 / 通行证签发与核对（纯函数）
  ...
  tests/              单测与 e2e
    fixtures/gomoku/  编写标准的活样本（开发用，不进 tools/ 也就不进工具包）
    fixtures/tools/   注入组件的 e2e 夹具（dev + ?fixtureTools=1 时才进注册表）
...
npm run agent:test     # AI 助手（动作协议 / 权限门 / 流式解析 / 网络重试 / 对话持久化；无浏览器）
...
| 逻辑单测（smoke） | **573 项** | `npm run smoke` |
| 同步单测（sync） | **232 项** | `npm run sync:test` |
| AI 助手单测（agent） | **847 项** | `npm run agent:test` |
| 工具编写标准单测（gomoku 夹具） | **59 项** | `npm run gomoku:test` |
| 浏览器 e2e | **21 套件 / 1351 项** | `node tests/_run-all-e2e.mjs` |

e2e 用本机 Edge（与 Tauri 的 WebView2 同源），覆盖 20 个面：
...
background     壁纸（内置 + 自定义上传）/ 遮罩 / 主题色调 / 从壁纸取色
...
agent-sandbox  沙箱真跑（jsdom 验不了的那一段）/ 通行证门 / 注入组件挂载与 task.get
```
